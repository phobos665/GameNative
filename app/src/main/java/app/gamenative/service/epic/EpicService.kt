...
class EpicService : Service() {
    ...
    companion object {
        ...
        suspend fun checkForUpdates(context: Context, gameId: Int?): Result<Int> {
            return getInstance()?.epicManager?.checkForUpdates(context, gameId)
                ?: Result.failure(IllegalStateException("EpicService not initialized"))
        }
